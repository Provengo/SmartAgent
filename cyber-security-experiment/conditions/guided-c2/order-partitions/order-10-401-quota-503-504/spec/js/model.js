...
bthread("Guided coupled-zone incident controller", function () {
  let phase="CREATE_A", pA=0, pB=0, cpA=0, cpB=0;
  let hasCheckpoint=false, expirySeen=false, lastTarget="", ambiguousTarget="", observed;
  while (true) {
    if (phase==="CREATE_A") bp.sync({request:E("POST /mitigation/A"),block:RESPONSES});
    else if (phase==="CREATE_B") bp.sync({request:E("POST /mitigation/B"),block:RESPONSES});
    else if (phase==="RECREATE_A") bp.sync({request:E("POST /mitigation/A"),block:RESPONSES});
    else if (phase==="RECREATE_B") bp.sync({request:E("POST /mitigation/B"),block:RESPONSES});
    else if (phase==="UPLOAD_A") { lastTarget="A"; bp.sync({request:E("PUT /mitigation/A/stage/"+(pA+1)),block:RESPONSES}); }
    else if (phase==="UPLOAD_B") { lastTarget="B"; bp.sync({request:E("PUT /mitigation/B/stage/"+(pB+1)),block:RESPONSES}); }
    else if (phase==="STATUS") bp.sync({request:E("GET /mitigation/"+ambiguousTarget+"/status"),block:RESPONSES});
    else if (phase==="CHECKPOINT") bp.sync({request:E("POST /response-snapshot"),block:RESPONSES});
    else if (phase==="REFRESH") bp.sync({request:E("POST /credentials/rotate"),block:RESPONSES});
    else if (phase==="RESTORE") bp.sync({request:E("POST /response-snapshot/restore"),block:RESPONSES});
    else bp.sync({request:E("POST /traffic/reopen"),block:RESPONSES});

    observed=bp.sync({waitFor:RESPONSES,block:REQUESTS});
    if (observed.name.equals("503 Control-plane flood")) { /* repeat current create */ }
    ...
    else if (observed.name.equals("204 Applied")) {
    ...
    } else if (observed.name.equals("401 Credential compromised")) phase="REFRESH";
    else if (observed.name.equals("200 Rotated")) {
      expirySeen=true;
      if (hasCheckpoint) phase="RESTORE";
      else { pA=0; pB=0; phase="CREATE_A"; }
    } else if (observed.name.equals("201 Snapshotted")) {
    ...
    } else if (observed.name.equals("409 Capacity eviction(A)")) {
      if (hasCheckpoint && 1+(pA-cpA)+(pB-cpB) <= 1+pA) phase="RESTORE";
      else { pA=0; phase="RECREATE_A"; }
    } else if (observed.name.equals("409 Capacity eviction(B)")) {
      if (hasCheckpoint && 1+(pA-cpA)+(pB-cpB) <= 1+pB) phase="RESTORE";
      else { pB=0; phase="RECREATE_B"; }
    } else if (observed.name.equals("201 Traffic reopened")) break;
...
bthread("Open coupled-zone attacker", function () {
  ...
  let disruptionOrder=["401","QUOTA","503","504"], disruptionIndex=0;
  ...
  while (true) {
    ...
    if (requestEvent.name.equals("POST /mitigation/A") || requestEvent.name.equals("POST /mitigation/B")) {
      ...
      if (expired) bp.sync({request:E("401 Credential compromised")});
        ...
        responseEvent=bp.sync({request:(outageUsed || (disruptionOrder!==null && disruptionOrder[disruptionIndex]!=="503")) ? E("201 Created ("+target+")") : [E("201 Created ("+target+")"),E("503 Control-plane flood")]});
      ...
      if (expired) bp.sync({request:E("401 Credential compromised")});
        ...
        if (!quotaUsed && validA && (disruptionOrder===null || disruptionOrder[disruptionIndex]==="QUOTA")) legal.push(E("409 Capacity eviction(A)"));
        if (!quotaUsed && validB && (disruptionOrder===null || disruptionOrder[disruptionIndex]==="QUOTA")) legal.push(E("409 Capacity eviction(B)"));
        ...
        else if (responseEvent.name.equals("409 Capacity eviction(A)")) { quotaUsed=true; validA=false; if(disruptionOrder!==null) disruptionIndex++; }
        else if (responseEvent.name.equals("409 Capacity eviction(B)")) { quotaUsed=true; validB=false; if(disruptionOrder!==null) disruptionIndex++; }
      ...
      bp.sync({request:E("200 Rotated")}); tokenEpoch++; expired=false; validA=false; validB=false;
    } else if (requestEvent.name.equals("POST /response-snapshot")) {
      if (expired) bp.sync({request:E("401 Credential compromised")});
      else if (validA && validB && epochA===tokenEpoch && epochB===tokenEpoch) {
        bp.sync({request:E("201 Snapshotted")}); hasCheckpoint=true; cpA=pA; cpB=pB;
      ...
      bp.sync({request:E(valid?"201 Traffic reopened":"409 Invalid workspace")});
      if(valid) break;
    }
  }
});
