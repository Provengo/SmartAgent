...
/** Stores current trace. Used for creating metadata  */
let trace=[];
...
function documentEvent( event ) {
    ...
    trace.push(event);      // store event in a trace for metadata generation.
    ...
    if ( d ) {
        ...
        } else if ( ( typeof EventCategory !== "undefined") && EventCategory?.any.contains(event) ) {
            EventCategory.addTestBookStep(event, "html");
 ...
 */
function endTrace() {
    TEST_SCENARIO.addMetadataLine("Event count: " + trace.length);

    // Attempt to create a meaningful title based on
    // the events in the current scenario.
    let title = ScenarioUtils.createTitle.byStateMachine(trace);
    if ( ! title ) {
        title = ScenarioUtils.createTitle.bySplitters(trace);
    }
    if ( title ){
        TEST_SCENARIO.setTitle(title);
    }
}
