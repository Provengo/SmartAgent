...
⚠️ NOTE: In the below listings, we assume that `provengo` is in the system's PATH variable, and that `C:\Users\geraw\provengo\SmartAgent\runs\fresh-agent-002\warehouse-controller` is the path to this directory.
...
Perform a single run through the specification. Good for "Sanity checks", i.e. to see examples of what can happen.
    ...
    provengo run --dry C:\Users\geraw\provengo\SmartAgent\runs\fresh-agent-002\warehouse-controller
...
Draw the specification in a PDF file.
    ...
    provengo analyze -f pdf C:\Users\geraw\provengo\SmartAgent\runs\fresh-agent-002\warehouse-controller
...
Sample 10 scenarios into a file. The scenarios are stored in a file called `samples.json` (this can be changed using the `-o`/`--output-file` switch).
    ...
    provengo sample --overwrite --size 10 C:\Users\geraw\provengo\SmartAgent\runs\fresh-agent-002\warehouse-controller
...
**Requires running `sample` first** (the previous command)**.**
    ...
    provengo ensemble --size 5 C:\Users\geraw\provengo\SmartAgent\runs\fresh-agent-002\warehouse-controller
...
Draw the specification, and highlight the traces in the optimized test suite create by the previous command.
    ...
    provengo analyze -f pdf --highlight ensemble.json C:\Users\geraw\provengo\SmartAgent\runs\fresh-agent-002\warehouse-controller

### Create Test Scripts for Third Party Systems

Converts the runs in `ensemble.json` to automation test scripts.

    provengo gen-scripts -s ensemble.json C:\Users\geraw\provengo\SmartAgent\runs\fresh-agent-002\warehouse-controller
