...
bthread("Naive REST backup controller", function () {
  let response;
  while (true) {
  ...
  const committed = bp.sync({waitFor: [E("201 Created (backup committed)"), E("409 Invalid Session")], block: REQUESTS});
  bp.ASSERT(committed.name.equals("201 Created (backup committed)"),
    "STRATEGY_FAILURE: token refresh invalidated the reused backup session");
});
